...
**Enable firewall** toggle lets you switch Patchstack's firewall on/off. While it's off, your site is not protected with virtual patches or firewall rules. Other hardening settings (IP bans, country blocking, htaccess rules, etc. will still work). This should only be turned off for testing purposes.

**Enable auto-prepend firewall** feature is new and currently designated a beta feature. It allows Patchstack to inject the auto_prepend_file variable into the .htaccess file which will let the Patchstack firewall run in all PHP scripts across the entire web-server. This makes it possible to block vulnerabilities in PHP files which are loaded outside WordPress core itself. This feature only works on Apache servers.
