...
<Steps>
1. **Priority filter** - find vulnerabilities by patch priority. You can pick one or more of **Highly exploitable**, **Exploitable** and **No impact**.

2. **Exploited** - filter out the vulnerabilities that are known to be exploited.

3. **Outdated** - show vulnerabilities where a patched release exists, so the installed version is behind the fix. (This filter was previously labelled "Fix available".)
</Steps>
...
- **No impact** — not expected to be exploited, and never given a vPatch.
