...
Vulnerabilities section shows you a general overview about all the vulnerabilities currently present on each of your WordPress sites. Each vulnerability is counted once, however many of your sites it affects, so the numbers here match the vulnerability list further down the page, which shows one row per vulnerability with the number of affected sites on it. The vulnerabilities are divided into three different groups, which indicate the patching priority:
